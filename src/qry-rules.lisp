...
(defmacro rqry (g &key (lim 1000) rules then)
  (declare (symbol g) (list rules))
  "example rules:
  ((*st-reach (?x ?y) (?x _ ?y)) ; trivial rule
   (*st-reach (?x ?y) (and (?x _ ?z) (*st-reach ?z ?y))) ; linear rule
   (*li-reach (?x ?u) (and (*st-reach ?x _) (?x ?u _)))) ; simple rule
  "
    ...
    (let ((all-names (undup (mapcar #'car rules))))
      (labels
        ((rule/qry (a r) `(qry ,g :pairs t :select ,a :where ,(rec/repl/f r)))
         ...
         (repl/f (r) ; what args should we use?
         `(:f (rules/prev-matches ,(car r) ,@(pairs/qt (find-name-args (car r)) (cdr r)))))
         (rec/repl/f (r)
           (cond ((atom r) r)
