...
modes:
  ...
  (let ((modes (valid-modes :split! modes *dir-modes*))
        (cc `(setf ,c* (the pn ,c)))
        (-> `(let ((,props* (props-as-list (@prop ,g (list ,a* ,b*)))))
               (del! ,g ,a* ,b*)
               (add! ,g ,a* ,c* ,props*)
               (add! ,g ,c* ,b* ,props*)))
        (<- `(let ((,props* (props-as-list (@prop ,g (list ,b* ,a*)))))
               (del! ,g ,b* ,a*)
               (add! ,g ,b* ,c* ,props*)
               (add! ,g ,c* ,a* ,props*))))
    `(let* ((,a* ,a) (,b* ,b) ,c*
     ...
     (let ((,g ,g*))
       ...
       ((,ht (&optional (fx #'eql))
        ...
        (,madd (fx hm a b &optional (v t))
          (declare (function fx) (hash-table hm))
          (setf (gethash a hm)
                (fset:with (gethash a hm (or (@ (funcall fx ,g) a)
                                             (fset:empty-map)))
                           b v)))
        (,sadd (fx hm a b)
          (declare (function fx) (hash-table hm) (keyword a) (list b))
          (setf (gethash a hm)
                (fset:with (gethash a hm (or (@ (funcall fx ,g) a)
                                             (fset:empty-set)))
                           b)))
